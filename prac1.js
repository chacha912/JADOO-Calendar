...
const initCalendar = () => {
  ...
  for (let i = firstDay - 1; i >= 0; i -= 1) {
    temp += `
            <div class="calendar-date unactive" data-date=${convertDateToString(
              currentYear,
              currentMonth - 1,
              i
            )}>
  ...
  for (let i = 1; i <= 6 - lastDay; i += 1) {
    temp += `
        <div class="calendar-date unactive ${
          i === 1 ? 'standard' : ''
...
const changeNextMonth = () => {
  ...
  for (let i = 1; i <= 6 - lastDay; i += 1) {
    temp += `
      ...
      i === 1
        ? `<span class="--hide">${
            lastYear + 1 > 12 ? lastYear + 1 : currentYear
          }</span><span class="--hide">${
...
const changePrevMonth = () => {
  ...
  for (let i = 1; i <= lastDate - (lastDay === 6 ? 0 : lastDay) - 1; i += 1) {
    temp += `
        <div class="calendar-date ${
          i === 1 ? 'standard' : ''
        } unactive" data-date=${convertDateToString(firstYear, firstMonth, i)}>
...
const addDataDomTree = ({ date, type, content }) => {
  ...
  $date.lastElementChild.insertAdjacentHTML(
    'beforeend',
    '<li class="item item-todo">' +
      typeContent +
      `<button class="item-control-btn" aria-label="아이템컨트롤러">
